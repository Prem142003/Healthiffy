        ...
        <div className="grid gap-6 xl:grid-cols-[420px_1fr]">
          ...
          <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
            ...
            {loading ? <p className="p-5 text-sm text-slate-600">Loading plans...</p> : plans.length === 0 ? <p className="p-5 text-sm text-slate-600">No plans created.</p> : (
              <div className="overflow-x-auto"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-slate-100 text-slate-700"><tr><th className="px-4 py-3">Plan</th><th className="px-4 py-3">Branches</th><th className="px-4 py-3">Price</th><th className="px-4 py-3">Schedule</th><th className="px-4 py-3">Actions</th></tr></thead><tbody className="divide-y divide-slate-200">{plans.map((plan) => <tr key={plan._id}><td data-label="Plan" className="px-4 py-3"><div className="font-medium text-slate-950">{plan.name}</div><div className="text-xs text-slate-500">{plan.menuItem?.name}</div>{!plan.isActive && <span className="mt-1 inline-block rounded-full bg-red-50 px-2 py-1 text-xs font-semibold text-red-700">Inactive</span>}</td><td data-label="Branches" className="px-4 py-3">{plan.branches.map((branch) => branch.name).join(', ')}</td><td data-label="Price" className="px-4 py-3">₹{plan.price}</td><td data-label="Schedule" className="px-4 py-3">{plan.totalMeals} meals / {plan.durationDays} days</td><td data-label="Actions" className="px-4 py-3"><div className="flex gap-2"><button className="rounded-md border border-slate-300 px-3 py-1 text-xs font-medium" onClick={() => startEdit(plan)}>Edit</button><button className="rounded-md border border-slate-300 px-3 py-1 text-xs font-medium" onClick={() => deactivate(plan)}>{plan.isActive ? 'Deactivate' : 'Activate'}</button></div></td></tr>)}</tbody></table></div>
            )}
          </section>
        </div>
      </section>
